...
Also serves as the project overview section in the PDF — the architecture diagram is
injected here so readers get a visual of the system before diving into the file listing.
...
Diagram: Architecture diagram — generated by `architecture_to_html.py`, injected automatically
into this section by `build_pdf.py`. No extra steps needed after running `architecture_to_html.py`.
