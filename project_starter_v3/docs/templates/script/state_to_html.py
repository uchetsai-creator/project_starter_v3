...
def parse_state(content):
    ...
    if '```state' in content:
        _m = re.search(r'```state\s*(.*?)```', content, re.DOTALL)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 state_to_html.py <input.md> [-o output.html]"); sys.exit(1)
    ...
    blocks = list(re.finditer(r'```state\s*(.*?)```', content, re.DOTALL))
    if not blocks:
        print("No ```state block found. Check your syntax."); sys.exit(1)

    # Strip existing suffix from base stem to avoid doubling (e.g. file-state-state.html)
    base_stem = os.path.splitext(base_output or input_path)[0]
    if base_stem.endswith('-state'):
        base_stem = base_stem[:-len('-state')]

    generated = 0

    for idx, match in enumerate(blocks):
        result = parse_state(match.group(1))
        title  = result[0] or ""
        ...
        output_path = out_stem + '-state.html'
        svg_path    = out_stem + '-state.svg'

        svg        = build_svg(*result)
        html_title = title or 'State Diagram'
