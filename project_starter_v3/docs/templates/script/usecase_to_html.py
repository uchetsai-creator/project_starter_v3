...
def parse_usecase(content):
    block_match = re.search(r'```usecase\s*(.*?)```', content, re.DOTALL)
...
def main():
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        content = f.read()
    title, actors, usecases, relations = parse_usecase(content)
    if not actors and not usecases:
        print("No actors or use cases found."); sys.exit(1)
    svg = build_svg(title, actors, usecases, relations)
    if not output_path:
        output_path = os.path.splitext(input_path)[0] + '-usecase.html'
    svg_path = os.path.splitext(output_path)[0] + '.svg'
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(HTML_TEMPLATE.replace('{title}', title or 'Use Case Diagram').replace('{svg_content}', svg))
